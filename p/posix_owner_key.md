# Function: <code>posix_owner_key</code>

## Status
<b>Regular</b>
<ul>
<li>
<details>
<summary>In <code>4.4</code>: Full Inline ⚠️</summary>

**Collision:** Unique Static

**Inline:** Full

**Transformation:** False

**Instances:**

```
In fs/locks.c (ffffffff8126040c)
Location: fs/locks.c:585
Inline: True
Inline callers:
  - fs/locks.c:__posix_lock_file
```
</details>
</li>
<li>
<details>
<summary>In <code>4.8</code>: Full Inline ⚠️</summary>

**Collision:** Unique Static

**Inline:** Full

**Transformation:** False

**Instances:**

```
In fs/locks.c (ffffffff8128cc76)
Location: fs/locks.c:612
Inline: True
Inline callers:
  - fs/locks.c:posix_lock_inode
```
</details>
</li>
<li>
<details>
<summary>In <code>4.10</code>: Full Inline ⚠️</summary>

**Collision:** Unique Static

**Inline:** Full

**Transformation:** False

**Instances:**

```
In fs/locks.c (ffffffff812a266b)
Location: fs/locks.c:632
Inline: True
Inline callers:
  - fs/locks.c:posix_lock_inode
```
</details>
</li>
<li>
<details>
<summary>In <code>4.13</code>: Full Inline ⚠️</summary>

**Collision:** Unique Static

**Inline:** Full

**Transformation:** False

**Instances:**

```
In fs/locks.c (ffffffff812b1b22)
Location: fs/locks.c:632
Inline: True
Inline callers:
  - fs/locks.c:posix_lock_inode
```
</details>
</li>
<li>
<details>
<summary>In <code>4.15</code>: Full Inline ⚠️</summary>

**Collision:** Unique Static

**Inline:** Full

**Transformation:** False

**Instances:**

```
In fs/locks.c (ffffffff812d5640)
Location: fs/locks.c:649
Inline: True
Inline callers:
  - fs/locks.c:posix_lock_inode
```
</details>
</li>
<li>
<details>
<summary>In <code>4.18</code>: Full Inline ⚠️</summary>

**Collision:** Unique Static

**Inline:** Full

**Transformation:** False

**Instances:**

```
In fs/locks.c (ffffffff812fff49)
Location: fs/locks.c:649
Inline: True
Inline callers:
  - fs/locks.c:posix_lock_inode
```
</details>
</li>
<li>
<details>
<summary>In <code>5.0</code>: Full Inline ⚠️</summary>

**Collision:** Unique Static

**Inline:** Full

**Transformation:** False

**Instances:**

```
In fs/locks.c (ffffffff81315a68)
Location: fs/locks.c:701
Inline: True
Inline callers:
  - fs/locks.c:posix_lock_inode
```
</details>
</li>
<li>
<details>
<summary>In <code>5.3</code>: Full Inline ⚠️</summary>

**Collision:** Unique Static

**Inline:** Full

**Transformation:** False

**Instances:**

```
In fs/locks.c (ffffffff8133d3bc)
Location: fs/locks.c:699
Inline: True
Inline callers:
  - fs/locks.c:posix_lock_inode
  - fs/locks.c:__locks_insert_block
```
</details>
</li>
<li>
<details>
<summary>In <code>5.4</code>: Full Inline ⚠️</summary>

**Collision:** Unique Static

**Inline:** Full

**Transformation:** False

**Instances:**

```
In fs/locks.c (ffffffff8135596c)
Location: fs/locks.c:700
Inline: True
Inline callers:
  - fs/locks.c:posix_lock_inode
  - fs/locks.c:__locks_insert_block
```
</details>
</li>
<li>
<details>
<summary>In <code>5.8</code>: Full Inline ⚠️</summary>

**Collision:** Unique Static

**Inline:** Full

**Transformation:** False

**Instances:**

```
In fs/locks.c (ffffffff8139c645)
Location: fs/locks.c:700
Inline: True
Inline callers:
  - fs/locks.c:posix_lock_inode
  - fs/locks.c:__locks_insert_block
```
</details>
</li>
<li>
<details>
<summary>In <code>5.11</code>: Full Inline ⚠️</summary>

**Collision:** Unique Static

**Inline:** Full

**Transformation:** False

**Instances:**

```
In fs/locks.c (ffffffff813ae045)
Location: fs/locks.c:700
Inline: True
Inline callers:
  - fs/locks.c:posix_lock_inode
  - fs/locks.c:__locks_insert_block
```
</details>
</li>
<li>
<details>
<summary>In <code>5.13</code>: Full Inline ⚠️</summary>

**Collision:** Unique Static

**Inline:** Full

**Transformation:** False

**Instances:**

```
In fs/locks.c (ffffffff813b5583)
Location: fs/locks.c:700
Inline: True
Inline callers:
  - fs/locks.c:posix_lock_inode
  - fs/locks.c:__locks_insert_block
```
</details>
</li>
<li>
<details>
<summary>In <code>5.15</code>: Full Inline ⚠️</summary>

**Collision:** Unique Static

**Inline:** Full

**Transformation:** False

**Instances:**

```
In fs/locks.c (ffffffff8140527e)
Location: fs/locks.c:700
Inline: True
Inline callers:
  - fs/locks.c:posix_lock_inode
  - fs/locks.c:__locks_insert_block
```
</details>
</li>
<li>
<details>
<summary>In <code>5.19</code>: Full Inline ⚠️</summary>

**Collision:** Unique Static

**Inline:** Full

**Transformation:** False

**Instances:**

```
In fs/locks.c (ffffffff81478f6a)
Location: fs/locks.c:650
Inline: True
Inline callers:
  - fs/locks.c:posix_lock_inode
  - fs/locks.c:__locks_insert_block
```
</details>
</li>
<li>
<details>
<summary>In <code>6.2</code>: Full Inline ⚠️</summary>

**Collision:** Unique Static

**Inline:** Full

**Transformation:** False

**Instances:**

```
In fs/locks.c (ffffffff8150b89a)
Location: fs/locks.c:636
Inline: True
Inline callers:
  - fs/locks.c:posix_lock_inode
  - fs/locks.c:__locks_insert_block
```
</details>
</li>
<li>
<details>
<summary>In <code>6.5</code>: Full Inline ⚠️</summary>

**Collision:** Unique Static

**Inline:** Full

**Transformation:** False

**Instances:**

```
In fs/locks.c (ffffffff81542ffa)
Location: fs/locks.c:637
Inline: True
Inline callers:
  - fs/locks.c:posix_lock_inode
  - fs/locks.c:__locks_insert_block
```
</details>
</li>
<li>
<details>
<summary>In <code>6.8</code>: Full Inline ⚠️</summary>

**Collision:** Unique Static

**Inline:** Full

**Transformation:** False

**Instances:**

```
In fs/locks.c (ffffffff81578526)
Location: fs/locks.c:636
Inline: True
Inline callers:
  - fs/locks.c:posix_lock_inode
  - fs/locks.c:__locks_insert_block
```
</details>
</li>
</ul>
<b>Arch</b>
<ul>
<li>
<details>
<summary>In <code>arm64</code>: Full Inline ⚠️</summary>

**Collision:** Unique Static

**Inline:** Full

**Transformation:** False

**Instances:**

```
In fs/locks.c (ffff800010418570)
Location: fs/locks.c:700
Inline: True
Inline callers:
  - fs/locks.c:posix_lock_inode
  - fs/locks.c:__locks_insert_block
```
</details>
</li>
<li>
<details>
<summary>In <code>armhf</code>: Full Inline ⚠️</summary>

**Collision:** Unique Static

**Inline:** Full

**Transformation:** False

**Instances:**

```
In fs/locks.c (c05e2dec)
Location: fs/locks.c:700
Inline: True
Inline callers:
  - fs/locks.c:posix_lock_inode
  - fs/locks.c:__locks_insert_block
```
</details>
</li>
<li>
<details>
<summary>In <code>ppc64el</code>: Full Inline ⚠️</summary>

**Collision:** Unique Static

**Inline:** Full

**Transformation:** False

**Instances:**

```
In fs/locks.c (c000000000525e50)
Location: fs/locks.c:700
Inline: True
Inline callers:
  - fs/locks.c:posix_lock_inode
  - fs/locks.c:__locks_insert_block
```
</details>
</li>
<li>
<details>
<summary>In <code>riscv64</code>: Full Inline ⚠️</summary>

**Collision:** Unique Static

**Inline:** Full

**Transformation:** False

**Instances:**

```
In fs/locks.c (ffffffe0002bef80)
Location: fs/locks.c:700
Inline: True
Inline callers:
  - fs/locks.c:posix_lock_inode
  - fs/locks.c:__locks_insert_block
```
</details>
</li>
</ul>
<b>Flavor</b>
<ul>
<li>
<details>
<summary>In <code>aws</code>: Full Inline ⚠️</summary>

**Collision:** Unique Static

**Inline:** Full

**Transformation:** False

**Instances:**

```
In fs/locks.c (ffffffff8134df4c)
Location: fs/locks.c:700
Inline: True
Inline callers:
  - fs/locks.c:posix_lock_inode
  - fs/locks.c:__locks_insert_block
```
</details>
</li>
<li>
<details>
<summary>In <code>azure</code>: Full Inline ⚠️</summary>

**Collision:** Unique Static

**Inline:** Full

**Transformation:** False

**Instances:**

```
In fs/locks.c (ffffffff8133ec2c)
Location: fs/locks.c:700
Inline: True
Inline callers:
  - fs/locks.c:posix_lock_inode
  - fs/locks.c:__locks_insert_block
```
</details>
</li>
<li>
<details>
<summary>In <code>gcp</code>: Full Inline ⚠️</summary>

**Collision:** Unique Static

**Inline:** Full

**Transformation:** False

**Instances:**

```
In fs/locks.c (ffffffff8134ba1c)
Location: fs/locks.c:700
Inline: True
Inline callers:
  - fs/locks.c:posix_lock_inode
  - fs/locks.c:__locks_insert_block
```
</details>
</li>
<li>
<details>
<summary>In <code>lowlatency</code>: Full Inline ⚠️</summary>

**Collision:** Unique Static

**Inline:** Full

**Transformation:** False

**Instances:**

```
In fs/locks.c (ffffffff8135d845)
Location: fs/locks.c:700
Inline: True
Inline callers:
  - fs/locks.c:posix_lock_inode
  - fs/locks.c:__locks_insert_block
```
</details>
</li>
</ul>

## Differences
