# Function: <code>is_omap446x</code>

## Status
<b>Regular</b>
<ul>
<li>
In <code>4.4</code>: Absent ⚠️
</li>
<li>
In <code>4.8</code>: Absent ⚠️
</li>
<li>
In <code>4.10</code>: Absent ⚠️
</li>
<li>
In <code>4.13</code>: Absent ⚠️
</li>
<li>
In <code>4.15</code>: Absent ⚠️
</li>
<li>
In <code>4.18</code>: Absent ⚠️
</li>
<li>
In <code>5.0</code>: Absent ⚠️
</li>
<li>
In <code>5.3</code>: Absent ⚠️
</li>
<li>
In <code>5.4</code>: Absent ⚠️
</li>
<li>
In <code>5.8</code>: Absent ⚠️
</li>
<li>
In <code>5.11</code>: Absent ⚠️
</li>
<li>
In <code>5.13</code>: Absent ⚠️
</li>
<li>
In <code>5.15</code>: Absent ⚠️
</li>
<li>
In <code>5.19</code>: Absent ⚠️
</li>
<li>
In <code>6.2</code>: Absent ⚠️
</li>
<li>
In <code>6.5</code>: Absent ⚠️
</li>
<li>
In <code>6.8</code>: Absent ⚠️
</li>
</ul>
<b>Arch</b>
<ul>
<li>
In <code>arm64</code>: Absent ⚠️
</li>
<li>
<details>
<summary>In <code>armhf</code>: Duplicate, Full Inline ⚠️</summary>

**Collision:** Static Duplication

**Inline:** Full

**Transformation:** False

**Instances:**

```
In arch/arm/mach-omap2/omap4-common.c (c1514eb8)
Location: arch/arm/mach-omap2/soc.h:156
Inline: True
Inline callers:
  - arch/arm/mach-omap2/omap4-common.c:omap_gic_of_init
```
```
In arch/arm/mach-omap2/omap-smp.c (c1515308)
Location: arch/arm/mach-omap2/soc.h:156
Inline: True
Inline callers:
  - arch/arm/mach-omap2/omap-smp.c:omap4_smp_prepare_cpus
```
```
In arch/arm/mach-omap2/omap-mpuss-lowpower.c (c1515864)
Location: arch/arm/mach-omap2/soc.h:156
Inline: True
Inline callers:
  - arch/arm/mach-omap2/omap-mpuss-lowpower.c:omap4_mpuss_early_init
  - arch/arm/mach-omap2/omap-mpuss-lowpower.c:omap4_mpuss_init
```
```
In arch/arm/mach-omap2/pm44xx.c (c1515ea4)
Location: arch/arm/mach-omap2/soc.h:156
Inline: True
Inline callers:
  - arch/arm/mach-omap2/pm44xx.c:omap4_pm_init_early
```
```
In arch/arm/mach-omap2/voltagedomains44xx_data.c (c1517afc)
Location: arch/arm/mach-omap2/soc.h:156
Inline: True
Inline callers:
  - arch/arm/mach-omap2/voltagedomains44xx_data.c:omap44xx_voltagedomains_init
```
</details>
</li>
<li>
In <code>ppc64el</code>: Absent ⚠️
</li>
<li>
In <code>riscv64</code>: Absent ⚠️
</li>
</ul>
<b>Flavor</b>
<ul>
<li>
In <code>aws</code>: Absent ⚠️
</li>
<li>
In <code>azure</code>: Absent ⚠️
</li>
<li>
In <code>gcp</code>: Absent ⚠️
</li>
<li>
In <code>lowlatency</code>: Absent ⚠️
</li>
</ul>

## Differences
