# Function: <code>bpf_lsm_binder_transaction</code>
...
```c
int bpf_lsm_binder_transaction(struct task_struct *from, struct task_struct *to);
```
...
```c
int bpf_lsm_binder_transaction(const struct cred *from, const struct cred *to);
```
