# Function: <code>fake_signal_wake_up</code>

## Status
<b>Regular</b>
<ul>
<li>
<details>
<summary>In <code>4.4</code>: Full Inline ⚠️</summary>

**Collision:** Unique Static

**Inline:** Full

**Transformation:** False

**Instances:**

```
In kernel/freezer.c (ffffffff810e9d4e)
Location: kernel/freezer.c:97
Inline: True
Inline callers:
  - kernel/freezer.c:freeze_task
```
</details>
</li>
<li>
<details>
<summary>In <code>4.8</code>: Full Inline ⚠️</summary>

**Collision:** Unique Static

**Inline:** Full

**Transformation:** False

**Instances:**

```
In kernel/freezer.c (ffffffff810f0aa0)
Location: kernel/freezer.c:97
Inline: True
Inline callers:
  - kernel/freezer.c:freeze_task
```
</details>
</li>
<li>
<details>
<summary>In <code>4.10</code>: Full Inline ⚠️</summary>

**Collision:** Unique Static

**Inline:** Full

**Transformation:** False

**Instances:**

```
In kernel/freezer.c (ffffffff810f7c00)
Location: kernel/freezer.c:97
Inline: True
Inline callers:
  - kernel/freezer.c:freeze_task
```
</details>
</li>
<li>
<details>
<summary>In <code>4.13</code>: Full Inline ⚠️</summary>

**Collision:** Unique Static

**Inline:** Full

**Transformation:** False

**Instances:**

```
In kernel/freezer.c (ffffffff810f9ad0)
Location: kernel/freezer.c:97
Inline: True
Inline callers:
  - kernel/freezer.c:freeze_task
```
</details>
</li>
<li>
<details>
<summary>In <code>4.15</code>: Full Inline ⚠️</summary>

**Collision:** Unique Static

**Inline:** Full

**Transformation:** False

**Instances:**

```
In kernel/freezer.c (ffffffff81104593)
Location: kernel/freezer.c:97
Inline: True
Inline callers:
  - kernel/freezer.c:freeze_task
```
</details>
</li>
<li>
<details>
<summary>In <code>4.18</code>: Full Inline ⚠️</summary>

**Collision:** Unique Static

**Inline:** Full

**Transformation:** False

**Instances:**

```
In kernel/freezer.c (ffffffff8110f393)
Location: kernel/freezer.c:97
Inline: True
Inline callers:
  - kernel/freezer.c:freeze_task
```
</details>
</li>
<li>
<details>
<summary>In <code>5.0</code>: Full Inline ⚠️</summary>

**Collision:** Unique Static

**Inline:** Full

**Transformation:** False

**Instances:**

```
In kernel/freezer.c (ffffffff8111a9df)
Location: kernel/freezer.c:99
Inline: True
Inline callers:
  - kernel/freezer.c:freeze_task
```
</details>
</li>
<li>
<details>
<summary>In <code>5.3</code>: Full Inline ⚠️</summary>

**Collision:** Unique Static

**Inline:** Full

**Transformation:** False

**Instances:**

```
In kernel/freezer.c (ffffffff811250e5)
Location: kernel/freezer.c:100
Inline: True
Inline callers:
  - kernel/freezer.c:freeze_task
```
</details>
</li>
<li>
<details>
<summary>In <code>5.4</code>: Full Inline ⚠️</summary>

**Collision:** Unique Static

**Inline:** Full

**Transformation:** False

**Instances:**

```
In kernel/freezer.c (ffffffff811310a5)
Location: kernel/freezer.c:94
Inline: True
Inline callers:
  - kernel/freezer.c:freeze_task
```
</details>
</li>
<li>
<details>
<summary>In <code>5.8</code>: Full Inline ⚠️</summary>

**Collision:** Unique Static

**Inline:** Full

**Transformation:** False

**Instances:**

```
In kernel/freezer.c (ffffffff81140475)
Location: kernel/freezer.c:94
Inline: True
Inline callers:
  - kernel/freezer.c:freeze_task
```
</details>
</li>
<li>
<details>
<summary>In <code>5.11</code>: Full Inline ⚠️</summary>

**Collision:** Unique Static

**Inline:** Full

**Transformation:** False

**Instances:**

```
In kernel/freezer.c (ffffffff8113c7e5)
Location: kernel/freezer.c:94
Inline: True
Inline callers:
  - kernel/freezer.c:freeze_task
```
</details>
</li>
<li>
<details>
<summary>In <code>5.13</code>: Full Inline ⚠️</summary>

**Collision:** Unique Static

**Inline:** Full

**Transformation:** False

**Instances:**

```
In kernel/freezer.c (ffffffff8113da35)
Location: kernel/freezer.c:94
Inline: True
Inline callers:
  - kernel/freezer.c:freeze_task
```
</details>
</li>
<li>
<details>
<summary>In <code>5.15</code>: Full Inline ⚠️</summary>

**Collision:** Unique Static

**Inline:** Full

**Transformation:** False

**Instances:**

```
In kernel/freezer.c (ffffffff81160bb5)
Location: kernel/freezer.c:94
Inline: True
Inline callers:
  - kernel/freezer.c:freeze_task
```
</details>
</li>
<li>
<details>
<summary>In <code>5.19</code>: Full Inline ⚠️</summary>

**Collision:** Unique Static

**Inline:** Full

**Transformation:** False

**Instances:**

```
In kernel/freezer.c (ffffffff81193936)
Location: kernel/freezer.c:94
Inline: True
Inline callers:
  - kernel/freezer.c:freeze_task
```
</details>
</li>
<li>
<details>
<summary>In <code>6.2</code>: Full Inline ⚠️</summary>

**Collision:** Unique Static

**Inline:** Full

**Transformation:** False

**Instances:**

```
In kernel/freezer.c (ffffffff811d1304)
Location: kernel/freezer.c:94
Inline: True
Inline callers:
  - kernel/freezer.c:freeze_task
```
</details>
</li>
<li>
<details>
<summary>In <code>6.5</code>: Full Inline ⚠️</summary>

**Collision:** Unique Static

**Inline:** Full

**Transformation:** False

**Instances:**

```
In kernel/freezer.c (ffffffff811e5574)
Location: kernel/freezer.c:94
Inline: True
Inline callers:
  - kernel/freezer.c:freeze_task
```
</details>
</li>
<li>
<details>
<summary>In <code>6.8</code>: Full Inline ⚠️</summary>

**Collision:** Unique Static

**Inline:** Full

**Transformation:** False

**Instances:**

```
In kernel/freezer.c (ffffffff811fb324)
Location: kernel/freezer.c:98
Inline: True
Inline callers:
  - kernel/freezer.c:freeze_task
```
</details>
</li>
</ul>
<b>Arch</b>
<ul>
<li>
<details>
<summary>In <code>arm64</code>: Full Inline ⚠️</summary>

**Collision:** Unique Static

**Inline:** Full

**Transformation:** False

**Instances:**

```
In kernel/freezer.c (ffff80001019840c)
Location: kernel/freezer.c:94
Inline: True
Inline callers:
  - kernel/freezer.c:freeze_task
```
</details>
</li>
<li>
<details>
<summary>In <code>armhf</code>: Full Inline ⚠️</summary>

**Collision:** Unique Static

**Inline:** Full

**Transformation:** False

**Instances:**

```
In kernel/freezer.c (c03e33b0)
Location: kernel/freezer.c:94
Inline: True
Inline callers:
  - kernel/freezer.c:freeze_task
```
</details>
</li>
<li>
<details>
<summary>In <code>ppc64el</code>: Full Inline ⚠️</summary>

**Collision:** Unique Static

**Inline:** Full

**Transformation:** False

**Instances:**

```
In kernel/freezer.c (c0000000001f8324)
Location: kernel/freezer.c:94
Inline: True
Inline callers:
  - kernel/freezer.c:freeze_task
```
</details>
</li>
<li>
<details>
<summary>In <code>riscv64</code>: Full Inline ⚠️</summary>

**Collision:** Unique Static

**Inline:** Full

**Transformation:** False

**Instances:**

```
In kernel/freezer.c (ffffffe0001290c2)
Location: kernel/freezer.c:94
Inline: True
Inline callers:
  - kernel/freezer.c:freeze_task
```
</details>
</li>
</ul>
<b>Flavor</b>
<ul>
<li>
<details>
<summary>In <code>aws</code>: Full Inline ⚠️</summary>

**Collision:** Unique Static

**Inline:** Full

**Transformation:** False

**Instances:**

```
In kernel/freezer.c (ffffffff81129855)
Location: kernel/freezer.c:94
Inline: True
Inline callers:
  - kernel/freezer.c:freeze_task
```
</details>
</li>
<li>
<details>
<summary>In <code>azure</code>: Full Inline ⚠️</summary>

**Collision:** Unique Static

**Inline:** Full

**Transformation:** False

**Instances:**

```
In kernel/freezer.c (ffffffff8111c0e5)
Location: kernel/freezer.c:94
Inline: True
Inline callers:
  - kernel/freezer.c:freeze_task
```
</details>
</li>
<li>
<details>
<summary>In <code>gcp</code>: Full Inline ⚠️</summary>

**Collision:** Unique Static

**Inline:** Full

**Transformation:** False

**Instances:**

```
In kernel/freezer.c (ffffffff81127575)
Location: kernel/freezer.c:94
Inline: True
Inline callers:
  - kernel/freezer.c:freeze_task
```
</details>
</li>
<li>
<details>
<summary>In <code>lowlatency</code>: Full Inline ⚠️</summary>

**Collision:** Unique Static

**Inline:** Full

**Transformation:** False

**Instances:**

```
In kernel/freezer.c (ffffffff81133bb5)
Location: kernel/freezer.c:94
Inline: True
Inline callers:
  - kernel/freezer.c:freeze_task
```
</details>
</li>
</ul>

## Differences
